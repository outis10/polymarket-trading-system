#!/usr/bin/env python3
"""
Aggregate subminute frame data into slot buckets and price-move ranges,
segmented by time window (day_type + time_frame) defined in config/time_windows.csv.

Supports any event duration (5m, 15m, 1h, 4h) via --event-minutes.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aggregate subminute data by slot and price ranges for any event duration."
    )
    parser.add_argument("--input", required=True, help="Input CSV or XLSX path.")
    parser.add_argument("--sheet", default="10sec", help="Sheet name (XLSX only).")
    parser.add_argument(
        "--output",
        default="backtest_output/pm_slot_ranges.csv",
    ...
    parser.add_argument(
        "--output-filtered",
        default="",
        help="Output CSV path for filtered rows (min-count applied).",
    )
    parser.add_argument(
        "--slot-seconds",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Event duration in minutes (default: 5). E.g. 5, 15, 60, 240.",
    )
    parser.add_argument(
        "--prob-source",
...
def build_ranges(price_move: pd.Series, step: float) -> tuple[pd.Series, pd.Series]:
    ...
    sup = inf + step
    return pd.Series(inf, index=price_move.index), pd.Series(sup, index=price_move.index)
...
def main() -> None:
    ...
    event_seconds = int(args.event_minutes) * 60
    ...
    if slot_seconds <= 0 or event_seconds % slot_seconds != 0:
        raise ValueError(
            f"--slot-seconds must be > 0 and divide {event_seconds} "
            f"({args.event_minutes}m) exactly."
        )
    ...
    print(f"Event: {args.event_minutes}m | Slot: {slot_seconds}s | Range step: {step}")
    ...
    floor_freq = f"{args.event_minutes}min"
    # ts_5m_block marks 5m boundaries — only valid when event_minutes==5.
    # For longer events always floor to the correct event window.
    if "ts_5m_block" in df.columns and args.event_minutes == 5:
        maybe_block = pd.to_numeric(df["ts_5m_block"], errors="coerce")
        if maybe_block.notna().all():
            df["_block_key"] = maybe_block.astype("int64")
            # ts_5m_block is stored in milliseconds by resample_klines_to_excel_subminute.py
            unit = "ms" if df["_block_key"].iloc[0] > 1e12 else "s"
            df["_block_start"] = pd.to_datetime(df["_block_key"], unit=unit, utc=True)
        else:
            df["_block_start"] = df["open_time_utc"].dt.floor(floor_freq)
            df["_block_key"] = (df["_block_start"].astype("int64") // 1_000_000).astype("int64")
    else:
        df["_block_start"] = df["open_time_utc"].dt.floor(floor_freq)
        df["_block_key"] = (df["_block_start"].astype("int64") // 1_000_000).astype("int64")
    ...
    max_slot = event_seconds // slot_seconds
    ...
    if not invalid_blocks.empty:
        ...
        raise ValueError(
            f"Detected invalid {args.event_minutes}m blocks before aggregation: "
    ...
    final_close_all = df_all.groupby("_block_key", sort=False)["close"].transform("last")
    ...
    grouped["count_of_klines_inside_range"] = grouped["count_of_klines_inside_range"].astype(int)

    if args.bayes_smoothing:
        n = grouped["count_of_klines_inside_range"].astype(float)
        wins = grouped["prob_up"] * n
        grouped["prob_up"] = (wins + args.prior_alpha) / (n + args.prior_alpha + args.prior_beta)
