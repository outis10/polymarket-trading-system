      ...
      <aside className={`sidebar ${sidebarOpen ? 'sidebar-open' : ''}`}>
        ...
        <div className="sidebar-section">
          ...
          <div style={{ marginTop: 8, fontSize: 13 }}>
            ...
            />
            {settings.mode === 'demo' ? 'Demo Mode' : 'Connected to Polymarket'}
          </div>
        </div>
        ...
        <div className="sidebar-section">
          ...
          <label className="chart-option">
            <input
              type="checkbox"
              checked={settings.chart_options?.includes('show_probability') ?? true}
              onChange={() => handleChartOptionToggle('show_probability')}
            />
            Show Probability %
          </label>
          <label className="chart-option">
            <input
              type="checkbox"
              checked={settings.chart_options?.includes('show_price_change') ?? true}
              onChange={() => handleChartOptionToggle('show_price_change')}
            />
            Show Price Change %
          </label>
          <label className="chart-option">
            <input
              type="checkbox"
              checked={settings.chart_options?.includes('show_order_book') ?? true}
              onChange={() => handleChartOptionToggle('show_order_book')}
            />
            Show Order Book
          </label>
        </div>
        ...
        <button className="refresh-btn" onClick={() => send({ type: 'refresh' })}>
          Refresh Now
        </button>
