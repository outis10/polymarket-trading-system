    ...
    <div className="book-levels-block">
      ...
      {levels.length === 0 ? (
        ...
        <div className="book-levels-list">
          {levels.map((lvl) => {
            const depth = Math.max(4, (lvl.shares / maxShares) * 100)
            return (
              <div key={`${title}-${lvl.price}`} className="book-level-item">
                <div
                  className={`book-level-depth ${tone === 'up' ? 'book-level-depth-up' : 'book-level-depth-down'}`}
                  style={{ width: `${depth}%` }}
                />
                <span className="book-level-price">{Math.round(lvl.price * 100)}¢</span>
                <span className="book-level-shares">{lvl.shares.toFixed(2)} sh</span>
                <span className="book-level-total">${lvl.total.toFixed(2)}</span>
              </div>
            )
          })}
        </div>
